...
def read_opus_file(path: str) -> Spectrum:
    ...
    snm = getattr(opus, "snm", "") or ""
    sfm = getattr(opus, "sfm", "") or ""
...
def read_jcamp(path: str) -> Spectrum:
    import re
    meta: Dict[str, str] = {}
    data_lines: List[str] = []
    in_xy = False
...
def read_any(path: str) -> Spectrum:
...
class App(_BaseTk):
    def __init__(self):
        ...
        self.line_color = "#1f77b4"  # matplotlib default kék (hex)
        self.files: List[str] = []
        self.idx: int = -1
        ...
        # Initial window size, scaled to the screen.
